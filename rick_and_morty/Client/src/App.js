...
function App() {
  ...
  async function onSearch(event) {
    event.preventDefault();
    try {
        const response = (await axios.get(`http://localhost:3001/rickandmorty/character/${input}`)).data
        let characterExist = characters.some((character) => character.id === response.id); //boolean
        if(characterExist === false && response.id) {
          setCharacters((oldChars) => [...oldChars, response]); //LO AGREGA AL ARRAY CHARACTERS con lo que ya tenia characters
        } else {
  ...
  //FUNCION PARA BUSCAR EN EL SERVER Y AGREGAR EL PERSONAJE ----PROMESAS---
